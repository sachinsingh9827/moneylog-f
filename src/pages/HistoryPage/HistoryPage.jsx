...
const HistoryPage = () => {
  ...
  const handleShareTransactionWhatsApp = () => {
    ...
    transactions.forEach((item, index) => {
      ...
      message += `${index + 1}. Date: ${date}\n   Time: ${time}\n   Amount: ₹${amount}\n   Type: ${type}\n\n`;
    });

    // Add any footer or notes
    message += `\n---\nGenerated by Moneylog App`;

    // Encode the message for WhatsApp
    const encodedMessage = encodeURIComponent(message);

    // You can hardcode a number or leave empty for user to select manually
  ...
  // Date & time format helpers (reuse your own)
  const formatDate = (dateStr) => {
    if (!dateStr) return "-";
    const date = new Date(dateStr);
    return `${String(date.getDate()).padStart(2, "0")}-${String(
    ...
    <div style={{ padding: 10 }} className="page">
      ...
      >
        ...
        >
          ...
          >
            {name}'s
            <button onClick={handleShareTransactionWhatsApp}>
              Share on WhatsApp
            </button>
          </Typography>{" "}
          {!showForm && (
